...
wireless_module_stage() {
    ...
    uci -q -c "$candidate_dir" del_list \
        "wireless.$WIRELESS_2G_DEVICE.hostapd_options=he_twt_responder=0" 2>/dev/null || :
    uci -q -c "$candidate_dir" add_list \
        "wireless.$WIRELESS_2G_DEVICE.hostapd_options=he_twt_responder=0" ||
        die 'failed to set the 2.4 GHz TWT responder hostapd option'
...
wireless_module_validate() {
    ...
    hostapd_options=$(uci_get "$candidate_dir" "wireless.$WIRELESS_2G_DEVICE.hostapd_options" 2>/dev/null || :)
    case " $hostapd_options " in
        *' he_twt_responder=0 '*) ;;
        *) die "wireless.$WIRELESS_2G_DEVICE.hostapd_options lacks he_twt_responder=0" ;;
    esac
