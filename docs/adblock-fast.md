...
The module replaces every existing `file_url` source with a fixed set of 20
enabled blocklists. These cover general advertising and tracking, native
Windows/Apple/Samsung tracking, Samsung-specific native tracking, smart TVs
and mobile devices, DNS bypass, gaming, CNAME trackers, and malicious domains.
Insecure downloads, non-ASCII entries, and automatic source-configuration
