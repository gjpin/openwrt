...
|---|---|---|---|
| `pixelmain` | `br-lan.1` | `192.168.1.1/24` | `192.168.1.100`–`.249` |
| `pixelguest` | `br-lan.2` | `192.168.2.1/24` | `192.168.2.100`–`.249` |
| `pixeliot` | `br-lan.3` | `192.168.3.1/24` | `192.168.3.100`–`.249` |
| `pixelsecondary` | `br-lan.4` | `192.168.4.1/24` | `192.168.4.100`–`.249` |

Every DHCP pool is enabled with a 12-hour lease. Only `pixelmain` requests an
IPv6 prefix assignment (`ip6assign=60`).

## Ethernet layout

VLAN 1 is untagged and primary on `lan1` through `lan4`. VLANs 2, 3, and 4 are
tagged on `lan1`, making it a mixed Main access port and trunk. They are not
